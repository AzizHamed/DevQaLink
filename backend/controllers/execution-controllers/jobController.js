...
const Job = require('../../models/scheduler-models/readyJobsModel');
...
async function processJob(readyJob) {
    console.log('Processing job4545:', readyJob);
    if (!readyJob) {
    ...
    try {
        ...
        if (!cluster) {
            console.log('No available cluster found for job:', readyJob.jobName);
